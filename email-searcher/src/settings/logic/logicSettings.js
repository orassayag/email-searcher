...
const logicSettings = {
    ...
    // This helps to detect special browser operations only for those users.
    ...
    // This flag field will determine if to generate real samples of emails rows from external json files or
    ...
    // The number of fake emails to create each time the searchStoreFakeEmailsStartSaga saga takes place.
    // This number needs to be very small, since it's creating the email and storing it on the database via API server call.
    ...
    // don't forget to add this number.
    ...
    // smart, and random search process operation to take place.
    ...
    // The maximum characters length of the search key within an Email class
    // that allowed it to be created.
    ...
    // user can navigate between email’s pages and have the pager component displayed.
    ...
    // The maximum number of emails that the user deletes on the user-emails page until
    // will be refreshed. The refresh operation of the page serve update to the counter
