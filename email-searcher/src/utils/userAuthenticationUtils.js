...
// or registration operation. This file was created to save space on the userAuthentication sagas
...
// This function checks if the user exists and then checks if the user is authenticated.
export const isUserAuthenticated = (userAuthentication) => {

    // If the user object is not null, perform a check within the
...
// This function verifies deep conditional authentication, for any action that there
// is a validation before / after calling the server to verify that only authenticated
// user is allowed to call the server. We can say that this function is checking more
// deeply that the user is authenticated, by adding more checks on UserAuthentication
...
export const setUserAuthentication = (userLocalStorageData) => {
    ...
    // Check existence of all required parameters. If one is missing or invalid, don't process the operation.
    ...
    // (To not override other possible parameters or to be overridden).
...
export const getUserLocalStorageData = (userInstances) => {
    ...
    // Check and validate the new instance that has been created. If one is missing don't process the operation.
...
// This function gets data from localStorage if it exists,
...
export const getTimeOutDelay = (expirationTime) => {
    ...
    // Validate the expirationTime parameter. If it does not exist, return the default number.
    if (!expirationTime) {
...
// This function validates the email and password in case of a login process.
export const validateLoginUserAuthenticationModeRequest = (userAuthenticationDataRequest) => {
    ...
    // Re-create the object and assign a general error message.
    if (!validationResult) {
...
// This function generates the error message to display to the user after the API call returned an error.
export const getUserErrorMessage = (error) => {

    // Console the user the error message if needed (For developers).
    console.error(error);

    // Check if the parameter of the error is existing. If not, return a general error.
