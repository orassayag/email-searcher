...
// are placed here. This file was created to save space on the userEmails sagas
...
import { toEmails } from '../models/conversion/email';
import ManageEmailValidationResult from '../models/helpers/ManageEmailValidationResult';
...
export const validateGetUserEmails = (userGetEmailsRequest) => {
    ...
    // If some are missing - Print console error to the user, and show a general error message to the user.
    ...
    // Validate the existence and validity of parameters in userEmails instances.
    ...
    // Validate updated pager value.
    if (userGetEmailsRequest.updatedPagerValue) {

        // If it exists, check if it is a valid number.
    ...
    // Validate updated counter value.
...
export const getUserEmails = (userGetEmailsRequest) => {
    ...
    // Check once again if the email count is empty. If so, once again don't do
    ...
    // Convert all data from the database of Firebase to an array of Emails class instances.
    ...
    // the server API to convert, unlike on the search-page (Home-page), where there is a limit
...
export const toggleEmailMoreInformationPanel = (toggleEmailData) => {
    ...
    // Check if the user click wasn't on the 'X' on the corner, or click on the link to the URL address, but in other places.
    // If click on 'X' or on the URL address, don't do anything, the user doesn't want to open the panel,
    // he wants to delete the email or visit the URL.
...
// This function gets a data request from the saga that is in charge of updating
// the count of emails to display on each page on the user-emails page, validates
// all the data and returns the validation result, ready to load the emails list once again.
export const userCounterUpdate = (userCounterUpdateData) => {
    ...
    // the maximum available number of counts that are defined in logic settings. If invalid, print error to the console.
...
// This function gets a data request from the saga that is in charge of updating
// the page navigation of emails to display on each page on the user-emails page, validates
// all the data and returns the validation result, ready to load the emails list once again.
...
// and continues for the next validation function if needed, or
...
export const validateToggleManageEmailModal = (toggleManageEmailModalData) => {
    ...
    // Verify that the email wasn't added / deleted (According to the specific case that the user processes into).
    // If so - Don't do nothing. Something is wrong, don't display the modal to the user.
    // If the email is already added / deleted and clicked on again - No need to do nothing.
...
// email item and the action that has not already been made.
...
// This function gets an array of emails and an email Id and returns the email
// item from the emails array by the Id. Later on, the result would validate
// on other functions. Since this logic repeats itself in many places, it's
// better to create one function that operates and performs this logic.
...
// that all parameters are valid and the email exists, and returns the
