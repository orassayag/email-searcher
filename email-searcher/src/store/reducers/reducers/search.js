...
// search only by functions that are implemented in this file reducer.
...
// This reducer function reset the state to the initial position and called when the page first loads.
// Why do we do this? Since the inputs of the state do not erase when the user redirects to another page.
...
// This reducer function is called when the user adds the email successfully to his
...
// This reducer function is called when the add email process is finished or
...
// This reducer function is called when the user input some comments
...
// If toggle - Update the value by previous value. If not - Set it to a new value.
...
// This reducer function updates the search key text-box when it changes.
...
            // Clear all errors in any of the search options or search keys.
...
// is changed, being successful (TEXT to URL and vice versa).
...
// and the search key, after all validation was successfully passed.
...
    // Prepare all the errors to display to the user, and the errors that need to be clear.
...
// This reducer function updates the updated emails list, turns off the loading to display the user the
// updated emails list after the successful search process has been finished.
