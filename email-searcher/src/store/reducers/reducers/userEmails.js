...
// status of the state. We update the state that relates the user only by
// functions that are implemented in this file reducer.
...
// successfully adds a new email to the user emails list.
...
// perform some logic that depends on this field after the first
...
// This reducer function displays a loading animation to the user while a
// call to the API server database has been made to get all the user
...
// a general error message about failing to get user emails to display
// due an unexpected error from the API server.
...
// This reducer is called when the user deletes the email successfully from his
...
export const onUserDeleteEmailModalToggleSuccess = (state, action) => {
    ...
    return updateObject({
        ...
        updatedProperties: {
            ...
            // toggle modal window closes) reset the delete email object instance.
            deleteEmail: action.deleteEmailData.deleteEmail ? action.deleteEmailData.deleteEmail : toManageEmail()
        }
    });
};

// This reducer is called when the delete email process is finished or
