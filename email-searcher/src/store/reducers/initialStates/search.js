...
// and store reducer. It contains an email list and all the relevant
// properties for all the functionality that needed for the search
// process and search options, and adding email to the user emails list.

import * as enums from '../../../enums/enums';
import { setMultiSearchOptions } from '../../../utils/searchUtils';
import { toSearchOption } from '../../../models/conversion/searchOption';
import { toManageEmail } from '../../../models/conversion/manageEmail';

// The initial state of the search process and add email parts.
export const initialState = {

    // This field determines if the search component is mounted or not.
    isComponentMounted: false,

    // This field represents the loading animation on the section of the
    ...
    // Will hold the final key search after the user decides to
    // perform the search process with the entered key, and
    // will display on the result's title.
    searchKeyInputValue: '',

    // Will hold the error message of the key search, if the user
    ...
    // unexpected error occurred while the user edits or enters
    // his preference on the search options panel.
    searchOptionsGeneralErrorMessage: null,

    // This field will hold the flag that determines if the
    // search options panel is displayed to the user or not.
    isShowSearchOptions: false,

    // This field will flag to display an error message instead
    // of the list of email in case of unexpected error during
    ...
    // after the user added an email to his emails list and waited
    // for the API server call and saga to end the process.
    isLoadingModal: false,

    // This field will hold the error message in case some unexpected
    // error happens while the user tries to add an email to his emails
    ...
    // or key-down on the text-area, this field is updated.
    addEmailComments: '',
...
// This logic repeats itself multiple times, so the best thing is to create this inner function.
