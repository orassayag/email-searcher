// This file contains all search related saga, that include calls to the API
...
import { toEmailModal, toEmails } from '../../../models/conversion/email';
...
        // Validate the type of the authentication required type. If invalid - Print error to the console and don't do anything.
...
// The modal will be open with all relevant details that are about to be added to users’ emails list.
...
    // Validate existence of validation results parameters and existing error parameters.
...
// This saga function called when the user changes any of the search options
// (Except for search mode) on the options search form, this saga raised.
...
    // Note that here we are not checking 'COMMENTS' or 'SEARCH_KEY' since they do not represent search options and are not relevant
...
// This saga function is called to handle the fake process for the first time
...
    // Perform success action (Remove loading and save the fake emails
...
// This saga handles the search process of emails.
...
        // Call the error saga to print a message to the user.
...
    // First check if it isFakeProcess. If true - It means that this is the first load of the page.
...
        // Call action to show an error message to the user.
...
        // Generate reset search results to perform showing error messages to the user.
        validationResult.searchOptionsList = [{
            searchOptionType: enums.SearchElementType.SEARCH_KEY,
            isValid: false,
            errorMessage: translate.error_general
...
    // Convert the results to email instances (If exists).
...
// This saga function called to perform on the first load of the search-page (Home-page), to perform inner
// logic functions, that the user will not notice them and required to be healthy
...
    // there is a call to searchStoreFakeEmailsStartSaga saga that does it. It will generate to
    // a logic settings number of emails inserted to the API Firebase database.
    if (settings.isGenerateFakeEmails) {

        // Call the generated fake emails and store them in the searchStoreFakeEmailsStartSaga saga.
...
    // to check if any fake emails exist on the database, if not, generate random fake emails and auto-store
    // them in the Firebase database. It will be needed, for example, when no API server to perform
    // real search operations are implemented, and a search operation will result in fake emails each time
    // a search operation will take place.
    if (settings.isVerifyFakeEmailsExistsOnDatabase) {

        // Call the verified existence of fake emails and store them in the searchVerifyFakeEmailExistsOnDatabaseStartSaga saga.
        yield call(searchVerifyFakeEmailExistsOnDatabaseStartSaga);
    }
}

// This inner saga function called (For development mode only), to generate fake emails and store them in the database of Firebase.
...
            // Send a fake email to the API database.
...
        // Call the API to perform the search emails from the server. Note that we do not pass any
        // parameters to the search process to perform a real search. The reason is that we assume
...
// This inner saga function is called to handle any case of error in the search process.
function* searchErrorStartSaga(validationResult) {

    // Validate for existence of validationResult parameter. If not exists -
    // Print error console to the user and don't do anything.
    if (!validationResult) {
...
    // Check if the error is a console error (Internal error that is not
...
// This saga function is called to run when the user clicks on the 'Add' button on
...
        // and to display the error message to the user.
...
// server when trying to add emails to the user's emails list -
...
// This inner saga function called when the email was successfully added to the user's emails list.
...
        // Call the error saga to show a general message to the user.
...
    // After a short time of delay, close the modal window and mark the email as added.
...
// This inner saga function is called to handle any case of error on the add email process.
...
// This inner saga function is called to handle any case of error on search error.
...
    // Print the error to the console if it exists.
