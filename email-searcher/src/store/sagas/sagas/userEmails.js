// This file contains all user emails related saga, that include calls to the API
...
import { toUserEmails } from '../../../models/conversion/userEmails';
import { toManageEmail } from '../../../models/conversion/manageEmail';
import UserEmailsTotalCountResult from '../../../models/helpers/UserEmailsTotalCountResult';

// This saga function is called to handle the user emails count process.
...
// This inner saga function is called to handle the operation of getting the updated user emails total count
...
    // Check if the results exist. If not - Something went wrong - Display error message to the user.
...
// This saga function is called to handle the adding of email to the user's emails
...
        // Re-create the object if it does not exist.
        userEmailsTotalCountResult = new UserEmailsTotalCountResult();
...
// This saga function is called to handle the process of getting the user emails list from the API
// server database, and update it on the stat to display the emails list to
// the user on the user-emails page.
export function* userGetEmailsProcessStartSaga(action) {

    // If it's the first load, Update the state that the component is mounted.
    if (action.getUserEmailsData.isFirstPageLoad) {

        // Call the first load reducer action to update the state with the component mounted to true.
...
    // Check if validationResult exists. If there it's not exist -
    // Print it to console and show the user a general error message.
    if (!validationResult) {

        // Assign the error details to print to the console,
        // and call the error saga to display the error message to the user.
...
    // Check if there is an unexpected error from the validationResult object and error fields.
    // If there is - Print it to console and show the user a general error message.
...
        // Call the no emails saga to display a general message to the user.
...
    // Prepare the emails from the database for conversion.
    action.getUserEmailsData.emailsDataBase = response.data;

    // Calculate the current relevant results according to the pager and count and return it.
...
// This inner saga function is called to handle any case of error in the emails process.
...
        // Print error to the console and don't do anything.
        // Assign the error details to print to the console.
        yield call(printErrorToConsole, {
            errorToConsole: {
                errorType: enums.ErrorType.MISSING,
...
    // (In case that the user clicked on the X icon to delete the email item on the corner of the email row).
...
// This inner saga function is called to handle the validation results of both userCounterUpdateStartSaga
...
    // Check if there is an unexpected error from the validationResult object and all the error fields.
    // If there is - Print it to console and show the user a general error message.
    if (validationResult.errorType || validationResult.parameters || validationResult.functionName) {

        // Call the error saga to show an error message to the user.
        yield call(userGetEmailsProcessErrorSaga(validationResult));
...
    // Check if the update state is needed. If not, don't do anything.
...
// This saga function is called to handle the update of emails count to display on
...
    // Call the userGetEmailsProcessStartSaga saga with an updated counter to display relevant emails.
...
// This saga function is called to handle the update of the emails pager to display on
...
// The modal will be open with all relevant details that are about to be deleted from users’ emails list.
...
    // If a missing or error message exists - User clicked on the X icon to
    // delete email modal, or something is wrong, don't display the modal to the user.
    // Either cases - Hide the delete email modal window.
...
// This saga function is called to run when the user clicks on the 'Delete' button on
...
    // validate all parameters and rules before sending an API request.
...
// This saga function is called to handle the deleting email to the user's emails
// list both on state and the API server and database.
...
    // Check that there are keys that exist with the given email Id.
...
// server when trying to delete emails from a user's emails list -
...
    // Print error to the console if there is one.
    yield call(printErrorToConsole, deleteEmailFailData);

    // Call the failure process action, to display a general error message to the user.
...
// This inner saga function called when the email was successfully deleted from the user's emails list.
...
        // Call the failed saga to display an error message to the user.
        yield call(userDeleteEmailFailSaga, validationResult);
...
    // Assign the updated data to the userEmails instance.
...
    // Will hold information data about the continuation of the saga actions.
    const reloadData = {

        // This field will determine if to just perform a 'fade-out' effect of the email,
        // or to refresh the whole page (After a logic settings number of delete emails
...
    // and check for emails left in a pager page. If user don't have pager (Less than
...
        // Since delete emails don't affect pager / counter, we want to initial the
        // page after some time of deleting emails. So, the logic is that after a number
        // from logic settings exceeded, deletes without refreshing the page, there will be
...
        // Check that any emails that are not in the action type of 'deleted' are left.
...
        // Assign more updated data to the userEmails instance.
...
        // If no need to reload the page - Perform fade-out effect and
        // remove the email from the list, and update the emails list in the state.

        // After a short time of delay, close the modal window and mark fade-out the deleted email.
