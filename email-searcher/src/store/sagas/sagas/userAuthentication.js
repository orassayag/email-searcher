// This file contains all user related saga, that include calls to the API
...
import { toUserInstances } from '../../../models/conversion/userInstances';
import { userGetEmailsTotalCountSaga } from '../sagas/userEmails';
import UISettings from '../../../settings/logic/UISettings';

// This saga function is called when the user enters input on
...
// This saga function is called when the user clicks
// on the 'login / register' link on the bottom of the user-authentication form.
...
// This saga function is called to handle in case the user decides to logout from the application.
export function* userAuthenticationLogoutStartSaga() {

    // Perform redirect to search-page (Home-page).
    yield call(userLogoutSuccessSaga);
}

// This saga function is called to set a delay and after that time logout the user (After the user token is expired).
...
// This saga function is called to handle the login / registration process.
export function* userAuthenticationProcessStartSaga(action) {
    ...
    // If the userAuthenticationMode is a registered process - The default emails count is 0.
    // ToDo: In Node.js API bring it altogether.
    ...
    // Perform success action (Remove any error and redirect the user to search-page (Home-page)).
...
// This saga function is called to check if the user is authenticated each time the application raises.
export function* userAuthenticationCheckStateStartSaga() {

    // Get the result if the user is authenticated or not.
    ...
    if (userInstances.userAuthentication.isUserAuthenticated()) {
        ...
        // Renew the delay time of the user token by the time left since the user got the user token.
        yield put(actions.onUserAuthenticationCheckTimeout(userInstances.userAuthentication.getUserTokenLeftTime()));

    } else {

        // If the user is not authenticated - Just in case - Delete any credentials may be stored.
        // And perform a redirect to search-page (Home-page) if not there.
        yield call(userLogoutSuccessSaga);
    }
}

// This inner saga function called to handle all the actions need to be called after the user
// logout from the site (Or his user authentication token has expired).
...
// This inner saga function is called to validate the email and the
...
// This inner saga function is called to handle any case of error on the user-authentication process.
