// Settings to generate fake data for default representation of emails when the page first load,
// without the need to call to the API server to perform unnecessary search process.
...
const fakeSettings = {
    ...
    // From this array can be generated random email address domain ending, to generates random emails addresses.
    domainsArray: ['gmail.com', 'hotmail.com', 'outlook.com', 'icloud.com', 'yahoo.com', 'aol.com'],

    // From this array can be generated random URL domain ending, to generates random links.
    linkEndsArray: ['com', 'org', 'net', 'int', 'edu', 'gov', 'mil'],

    // From this enum array, random fake search engine will be generated for the fake emails.
    searchEngineArray: Object.values(enums.SearchEngine),

    // The exact length of email Id of Email class.
    idStringLength: 20,

    // The minimum email's string characters length to generate fake email address.
    minimumEmailLength: 5,

    // The maximum email's string characters length to generate fake email address.
    maximumEmailLength: 20,

    // The minimum link's string characters length to generate fake link.
    minimumLinkLength: 10,

    // The maximum link's string characters length to generate fake link.
    maximumLinkLength: 30,

    // DATES
    // In the case of generating fake data - It's only creation date of email's row.
    ...
    // SEARCH KEYS
    // When generating fake search key, a random number of strings is selected, and represent a fake search key.

    // The minimum strings (Words) count of search key to random.
    minimumSearchKeyStringsCount: 2,

    // The maximum strings (Words) count of search key to random.
