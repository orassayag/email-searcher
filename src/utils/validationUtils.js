// This file implement all the function that validates inputs, like numbers, dates,
...
// some of cases in functions throw exceptions in case of an errors occurred.

import * as enums from '../enums/enums';
import translate from '../translate/translate';
import { getErrorParametersString } from '../utils/textUtils';

// This function validates the props from any component that are exists and valid.
export const validateParameters = (validateParametersData) => {

    // Check existence of the validateParametersData parameters. If not exists, throw new exception to the user.
    if (!validateParametersData || !validateParametersData.props || !validateArrayItems(validateParametersData.parametersList)) {

        // Throw exception with all relevant parameter’s names.
    ...
    validateParametersData.parametersList.forEach(parameter => {
        ...
        // Check the parameter exists. If not exists, throw new exception to the user.
        if (!validateParametersData.props[parameter]) {

            // If any of the parameter is missing - Throw exception with the specific parameters that is missing.
            throw new Error(translate.error_missing.replace('#params#', parameter));
        }
    });
};

// This function validate the props of a specific component
// and also validate button / input type on one of the props.
export const validateParametersAndType = (validateParametersData) => {
    ...
    validateParametersData.validationTypes.forEach((validationType, i) => {
        ...
        // Determine the function to validate on specific field.
        // Validate that the type of the field and check if is not invalid.
        switch (validationType) {
            case enums.ValidationFunctionType.INPUT:
                ...
                if (['text', 'email', 'password'].indexOf(validateParametersData.props[validateParametersData.targetParameters[i]]) < 0) {
                    ...
                    // Throw exception with relevant parameter name.
...
// This function validates the props of a specific component and also validate an array with specific length.
// Validate that given value is an array with a specific length. This function created
// since to the current production development, to PropTypes don't have validation on array's length.
export const validateParametersAndArray = (validateParametersData) => {
    ...
    // Check for type of array with specific count and check if arrays lengths not equals.
...
// This function validates a given string that exists in array list of specific type.
...
const baseValidateMultiValues = (multiValuesData) => {
    ...
    // Will hold the invalid value if would find any.
    let invalidValue = null;

    // Check for existence of the multiValuesData parameter. If not exits, stop any further action.
    ...
    if (multiValuesData.value) {
        ...
        // Check that the array exists and have items.
...
export const validateURL = (value) => {
    ...
    // Check that the given URL address has already have http or https. If no - Give it.
    const prefix = 'http://';
...
// This function validates that array exists and any items exists in that array.
...
// This short logic become a function since it's recreates in many places in the sagas.
...
// This function validates search option of domain, that the given value is a valid domain.
...
// This function validates search option of email key, that the given value is a valid email key.
