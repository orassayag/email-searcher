...
// This file was created to save space on the search sagas itself and to keep
...
// This function creates a copy of the search options and updates some of the elements
// inside it, and returns an updated copy of the search options. This function updates
...
export const setSearchOptions = (searchOptionsData) => {
    ...
    // Make the index of the search option.
...
// inside it, and returns an updated copy of the search options.
// This function updates multi search options.
...
// This function creates a copy of the search options and reset all the element values
// and returns an updated copy of the search options. This function updates multi search options.
...
// This function prepares all the errors to display the user, and the errors that need to be clear,
...
export const initialErrorValidationResults = (errorSearchOptionsData) => {
    ...
    // Error message of search key, error reference on mobile. Will be determining which
    ...
    // Search key error data. If this variable is not empty - There is an error with the search key.
    const searchKeyData = errorSearchOptionsData.find(src => src.searchOptionType === enums.SearchElementType.SEARCH_KEY);

    // Check if an error message exists on the key data.
    if (searchKeyData) {
    ...
    // Holds all the errors to update state about all the
...
const validateSearchOptions = (validateSearchOptionsData) => {
    ...
    // Validate existence of the validateSearchOptionsData parameters. If parameters do not exist - Don't do nothing.
    ...
    for (let i = 0; i < validateSearchOptionsData.searchData.searchOptions.length; i++) {
        ...
        if (!searchOption.value || (validateSearchOptionsData.searchData.searchMode === enums.SearchMode.URL &&
            ...
            // Continue to the next search option in the list.
            continue;
        }
        ...
        // Check if search mode data was found. If not found, continue to the next element.
        if (!searchModeData) {
        ...
        // Check if the validation results object exists. If not, print error to the console.
...
export const validateAddEmailProcessRequest = (addEmailData) => {
    ...
    // Validate that the creation date is a valid date. If not, print error to the console.
    ...
    // Validate that the user added date does not exist. If something went wrong, print error to the console.
    ...
    // Check if the user entered the search key and / or comments - In order to know if to assign the
    ...
    // Get the search mode that the user found the email according to. If not exists -
    // Something went wrong and the search is invalid, return a general error to display to the user.
    ...
    // Check that the emailSearchKey is not exceeded maximum length. If so, print error to the console.
    if (!validationUtils.validateCharactersLength({
    ...
    // Validate the data according to the current selected search mode. If not, print error to the console.
    ...
    if (validationResult.emailItem.emailComments) {
        ...
        // Check that the comments are not exceeded maximum length. If not, print error to the console.
        if (!validationUtils.validateCharactersLength({
            ...
            // Set the error message with the relevant error.
    ...
    // call the API server to add the email to the user's emails list.

    // Return the validation results.
    return validationResult;
};

// This function validates the search process request.
export const validateSearchRequest = (searchData) => {

    // Create base template result to return.
    // Note that we create an array of all the search options (Include main search key) to validate.
    ...
    // search key will take place only if the search key exists.
    ...
    // Check that value is entered in the text-box. If empty - Display error to the user.
    ...
    // Check if the search key is valid after the previous check.
    ...
    if (isSearchKeyIsValid) {
        ...
        // Validate the data according to the current selected search mode. If invalid - Display error to the user.
